...
    
    ...
    const punishmentTypes = ['warn', 'ban', 'mute', 'kick'];
    const userLogs = logs.filter(entry => 
        entry.user === actionData.user && punishmentTypes.includes(entry.type)
    );
    
    let nextId = userLogs.length + 1;
    let punishmentId = `#${nextId}`;

    if (actionData.type === 'unban') {
        const lastPunishment = userLogs.length > 0 ? userLogs[userLogs.length - 1] : { id: '#0' };
        punishmentId = `#${logs.length + 1}`;
    }
